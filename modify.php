...
if(isset($_POST['modify_values']))
{
	...
	// mysql_select_db("department",$link) or die ("Cannot select the database!");
	 $query="UPDATE info_package SET author='".$author."', title='".$title."', isbn_no='".$isbn_no."', date_pub='".$date_pub."', keywords='".$keywords."', call_no='".$call_no."', publisher='".$publisher."', place_pub='".$place_pub."' WHERE id='".$id."'";
...
?>
 <?php 
	...
	// mysql_select_db("department",$link) or die ("Cannot select the database!");
	 $query="SELECT * FROM info_package WHERE id='".$id."'";//used to get the values, from the db, they are shown in the form, for you to modify
		
		 $resource=mysql_query($query) or die ("An unexpected error occured while <b>updating</b> the record, Please try again!");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 ...
 <tr>
	...
	<form action="info_packageform.php" method="post">
	  ...
	  <tr>
        <td>Author</td>
        <td><input type="text" name="author" size="30"  value="<?php echo $rows['author'] ?>"/></td>
      </tr>
      <tr>
        <td>Title</td>
        <td><input type="text" name="title" size="30" value="<?php echo $rows['title'] ?>"/></td>
      </tr>
      <tr>
        <td>ISBN Number</td>
        <td><input type="text" name="isbn_no" size="30" value="<?php echo $rows['isbn_no'] ?>"/></td>
      </tr>
      <tr>
        <td>Year of Publication</td>
        <td><input type="year" name="date_pub" size="30" value="<?php echo $rows['date_pub'] ?>"/></td>
      </tr>
      <tr>
        <td>Keywords</td>
        <td><input type="text" name="keywords" size="30" value="<?php echo $rows['keywords'] ?>"/></td>
      </tr>
      <tr>
        <td>Call Number</td>
        <td><input type="text" name="call_no" size="30" value="<?php echo $rows['call_no'] ?>"/></td>
      </tr>
      <tr>
        <td>Publisher</td>
        <td><input type="text" name="publisher" size="30" value="<?php echo $rows['publisher'] ?>"/></td>
      </tr>
      <tr>
        <td>Place of Publication</td>
        <td><input type="text" name="place_pub" size="30" value="<?php echo $rows['place_pub'] ?>"/></td>
      </tr>
      <tr>
        <td></td>
        <td><input type="submit" name="modify_values" size="size""10" value="Modify" />
           </td>
      </tr>
	  </form>
    </table> 	 
